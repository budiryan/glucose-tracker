...
from .models import Glucose, Color
from .reports import (
    GlucoseCsvReport,
    GlucosePdfReport,
    ChartData,
...
@login_required
def chart_data_color_json(request):
    data = {}
    params = request.GET
    ...
    name = params.get('name', '')
    if name == 'avg_by_category':
        data['chart_data'] = ChartDataColor.get_avg_by_category(
            user=request.user, days=int(days))
    elif name == 'avg_by_day':
        data['chart_data'] = ChartDataColor.get_avg_by_day(
            user=request.user, days=int(days))
    elif name == 'level_breakdown':
        data['chart_data'] = ChartDataColor.get_level_breakdown(
            user=request.user, days=int(days))
    elif name == 'count_by_category':
        data['chart_data'] = ChartDataColor.get_count_by_category(
            user=request.user, days=int(days))
...
@login_required
def stats_json_color(request):
    data = {'stats': UserStatsColor(request.user).user_stats}
...
class ColorCreateView(LoginRequiredMixin, CreateView):
    model = Color
    success_url = '/dashboard-2/'
    template_name = 'glucoses/color_create.html'
    ...
    def form_valid(self, form):
        ...
        if form.cleaned_data['submit_button_type'] == 'submit_and_add':
            self.success_url = '/glucoses/add-color/'
            value = form.cleaned_data['value']
            messages.add_message(self.request, messages.SUCCESS,
                                 "Urine color '%s' successfully added. You may "
                                 "add another." % value)
...
class ColorUpdateView(LoginRequiredMixin, UpdateView):
    ...
    template_name = 'glucoses/color_update.html'
    ...
    def get(self, request, *args, **kwargs):
        ...
        if self.object.user != request.user:
            ...
            return super(ColorUpdateView, self).get(request, *args, **kwargs)

    def get_success_url(self):
        return reverse('dashboard-2')

    def get_object(self, queryset=None):
        try:
            obj = Color.objects.get(pk=self.kwargs['pk'])
    ...
    def post(self, request, *args, **kwargs):
        ...
        return super(ColorUpdateView, self).post(request, *args, **kwargs)
...
class ColorListJson(LoginRequiredMixin, BaseDatatableView):
    model = Color
    ...
    def render_column(self, row, column):
        ...
        if column == 'value':
            ...
            edit_url = reverse('color_update', args=(row.id,))
            ...
            delete_url = reverse('color_delete', args=(row.id,))
            print "debug:"
            print delete_url
            ...
            return super(ColorListJson, self).render_column(row, column)

    def get_initial_queryset(self):
        """
        Filter records to show only entries from the currently logged-in user.
        """
        return Color.objects.by_user(self.request.user)
